...
server <- function(input,output){
  ...
  input_length <- reactive({length(unlist(strsplit(input$text," ")))})
 ################################################### Start of Test If then in Shiny
  ...
  output$prediction <- renderText({
    if(input_length()>=3){
      ...
      final_bo_quadgrams()$word4
      
    } else if(input_length()==2) {
      bigram_prefix <- reactive({select_2gram(tolower(input$text))})
      ## 5.3.1 Look for observed 3-grams
      
      obs_bo_trigrams <- reactive({filter(en_us_all_count_3,word1==bigram_prefix()[1]&word2==bigram_prefix()[2])})
      ...
      obs_bo_bigrams <- reactive({filter(en_us_all_count_2,word1==bigram_prefix()[2])})
      ...
      total_bo_trigrams <- reactive({rbind(obs_bo_trigrams2(),adj_bo_trigrams2())})
      final_bo_trigrams <- reactive({arrange(total_bo_trigrams(),desc(prob))[1:3,]})
      final_bo_trigrams()$word3
    } else {
      unigram_prefix <- reactive({select_1gram(tolower(input$text))})
      obs_bo_bigrams <- reactive({filter(en_us_all_count_2,word1==unigram_prefix())})
      ...
      total_bo_bigrams <- reactive({rbind(obs_bo_bigrams2(),unigram_tail3())})
      final_bo_bigrams <- reactive({arrange(total_bo_bigrams(),desc(prob))[1:3,]})
      final_bo_bigrams()$word2
    }
  })
}
